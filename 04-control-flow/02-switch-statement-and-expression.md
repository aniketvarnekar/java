...
1. Rewrite this classic, fall-through-reliant `switch` statement as a modern `switch` expression using comma-separated case labels:
   ```java
   switch (month) {
   ...
   ```
2. Predict the exact printed output of the fall-through example from this topic if `day = 1` instead of `day = 2`, and explain your reasoning step by step.
3. Explain why `case someLocalVariable:` is illegal as a `switch` case label, tying your answer back to Module 03's compile-time constant discussion.
4. Write a `switch` expression (using `yield`) that, given an `int score`, prints a diagnostic message AND produces a letter grade string in the same branch for scores below 60.

---

**Previous:** [01 — If-Else & Conditional Logic](01-if-else-and-conditional-logic.md) · **Next:** [03 — While & Do-While Loops](03-while-and-do-while-loops.md)
