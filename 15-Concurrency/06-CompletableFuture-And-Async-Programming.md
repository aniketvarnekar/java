...
1. Write a `CompletableFuture` chain that fetches a value asynchronously, doubles it (`thenApply`), and prints the result (`thenAccept`).
2. Write two independent `CompletableFuture`s and combine their results using `thenCombine`.
3. Rewrite a chain step that itself returns a `CompletableFuture` using `thenApply` first (observe the awkward nested type), then correctly using `thenCompose` instead.
4. Add exception handling to a chain where the initial async operation might throw, using `exceptionally` to supply a fallback value.

---

**Previous:** [05 — Executors & Thread Pools](05-Executors-And-Thread-Pools.md) · **Next:** [07 — Concurrent Collections](07-Concurrent-Collections.md)
