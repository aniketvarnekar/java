...
1. Fix the `Counter` class from Topic 2 by adding `synchronized` to `increment()`, and explain precisely why this resolves both hazards discussed there.
2. Implement a simple producer-consumer `Buffer` class using `wait()`/`notifyAll()`, correctly using a `while` loop for the condition check.
3. Rewrite the same `Counter` using `ReentrantLock` instead of `synchronized`, correctly using `try`/`finally` for unlocking.
4. Construct a small, concrete two-lock deadlock scenario (as shown in this topic), then rewrite it using a consistent lock-acquisition order to eliminate the deadlock risk.

---

**Previous:** [02 — Race Conditions & the Java Memory Model](02-race-conditions-and-the-java-memory-model.md) · **Next:** [04 — Atomic Classes & CAS](04-atomic-classes-and-cas.md)
