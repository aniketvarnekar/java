...
1. Predict the output of this code, and explain your reasoning using the Stack/Heap trace style from this topic:
   ```java
   class Box { int value; }
   void modify(Box b) { b.value = 100; b = new Box(); b.value = 200; }
   Box box = new Box();
   box.value = 1;
   modify(box);
   System.out.println(box.value);
   ```
2. Explain precisely why "Java is pass-by-reference for objects" is an incorrect way to describe the behavior demonstrated in Exercise 1, even though object mutation IS visible to the caller.
3. Write a method `sum(int first, int... rest)` and explain why this parameter ordering is legal while `sum(int... rest, int first)` is not.

---

**Previous:** [00 — Module Overview](00-Module-Overview.md) · **Next:** [02 — Constructors](02-Constructors.md)
