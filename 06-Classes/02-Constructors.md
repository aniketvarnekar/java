...
1. Write a `Rectangle` class with a two-argument constructor (`width`, `height`) and an overloaded single-argument constructor that creates a square (using `this(...)` chaining to the two-arg constructor).
2. Explain why this code fails to compile, and fix it: `class Config { Config(String name) { this.name = name; } } Config c = new Config();`
3. Explain why a constructor cannot call both `this(...)` and `super(...)`, referencing what each is responsible for.

---

**Previous:** [01 — Class Members: Fields & Methods](01-Class-Members-Fields-And-Methods.md) · **Next:** [03 — The `this` Keyword](03-The-this-Keyword.md)
