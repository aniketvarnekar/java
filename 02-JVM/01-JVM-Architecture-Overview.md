...
1. Redraw the four-part JVM architecture diagram from memory, labeling what flows from one part to the next.
2. Without reading ahead, guess: which subsystem do you think is responsible for a `NoClassDefFoundError`? Which for a `StackOverflowError`? (We'll confirm in Topics 2 and 3.)
3. Explain, using the restaurant kitchen analogy or one of your own, why per-thread Stacks are kept separate from the shared Heap, rather than everything being stored in one big shared pool.

---

**Previous:** [00 — Module Overview](00-Module-Overview.md) · **Next:** [02 — Class Loader Subsystem](02-Class-Loader-Subsystem.md)
