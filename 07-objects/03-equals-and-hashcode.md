...
1. Write a correct `equals()` and `hashCode()` for a `Money` class with `long cents` and `String currency` fields, using `getClass()` and `Objects.hash(...)`.
2. Reproduce the `HashSet.contains()` failure demonstrated in this topic (override `equals()` but deliberately omit `hashCode()`), run it, and confirm the described broken behavior yourself.
3. Explain, step by step, why the `Point`/`ColorPoint` `instanceof`-based `equals()` example violates symmetry, and rewrite both classes' `equals()` methods using `getClass()` to fix it.
4. Explain precisely what could go wrong if you stored a mutable-field object in a `HashSet`, then later changed one of the fields used in that object's `hashCode()` — describe the exact failure mode.

---

**Previous:** [02 — `toString()`](02-tostring.md) · **Next:** [04 — Object Cloning](04-object-cloning.md)
