...
1. Write a stream pipeline with a `filter` containing a `System.out.println` inside its lambda, and demonstrate that nothing prints until a terminal operation is added and called.
2. Attempt to call two terminal operations on the same `Stream` object, and observe/explain the resulting exception.
3. Explain, in your own words, why `Stream.iterate(1, n -> n + 1)` must be bounded with `.limit(...)` before a terminal operation like `.collect(...)` can safely be used with it.

---

**Previous:** [00 — Module Overview](00-module-overview.md) · **Next:** [02 — Intermediate Operations](02-intermediate-operations.md)
