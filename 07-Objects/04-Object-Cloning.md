...
1. Implement a `Cloneable` `Rectangle` class with a `List<String> tags` field, demonstrate the shallow-copy bug (mutating the clone's `tags` affecting the original), then fix it with a proper deep-copying `clone()` override.
2. Rewrite the same `Rectangle` class using a copy constructor instead, and explain why this version doesn't require `implements Cloneable` or handling any checked exception.
3. Explain, precisely, why `Person copy = original;` is not a copy at all, connecting your answer to Module 02's reference/Heap model.

---

**Previous:** [03 — `equals()` and `hashCode()`](03-equals-And-hashCode.md) · **Next:** [05 — Object Lifecycle & Garbage Collection](05-Object-Lifecycle-And-Garbage-Collection.md)
