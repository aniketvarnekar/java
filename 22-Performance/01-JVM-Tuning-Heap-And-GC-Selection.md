...
1. Explain why a latency-sensitive production API service might deliberately set `-Xms` equal to `-Xmx`, rather than letting the heap grow dynamically.
2. For each scenario, choose an appropriate GC and justify it: (a) a nightly batch ETL job, (b) a real-time bidding service with a 32GB heap and strict SLA requirements, (c) a small internal CLI utility.
3. Explain why an unbounded Metaspace could be a genuine problem specifically in a containerized deployment with a strict memory limit.

---

**Previous:** [00 — Module Overview](00-Module-Overview.md) · **Next:** [02 — Profiling & Benchmarking](02-Profiling-And-Benchmarking.md)
