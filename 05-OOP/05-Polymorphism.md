...
1. Write a small `Shape` superclass with a `double area()` method, and two subclasses (`Circle`, `Rectangle`) each overriding it correctly with `@Override`. Write a loop over a `List<Shape>` containing both, printing each one's area, and explain why no type-checking is needed in the loop.
2. Explain, step by step, what the JVM actually does when `shape.area()` is called inside that loop, referencing dynamic method dispatch and each shape's method table.
3. Given `Employee e = new Intern(...);`, explain why `Manager m = (Manager) e;` compiles successfully but throws an exception at runtime, and rewrite it safely using `instanceof` pattern matching.
4. Explain, precisely, why `calc.add(5, 10)` and `calc.add(5.0, 10.0)` (given the overloaded `Calculator` from this topic) are resolved to different methods *before the program ever runs* — contrast this explicitly with how an overridden method's resolution timing differs.

---

**Previous:** [04 — Inheritance](04-Inheritance.md) · **Next:** [06 — Interfaces & Abstract Classes](06-Interfaces-And-Abstract-Classes.md)
