...
1. Given `int a = 5, b = -3;`, trace through the brace-less dangling-else example in this topic by hand and determine exactly what it prints — then rewrite it with braces to make the *opposite* behavior (else binding to the outer `if`) actually happen.
2. Explain why `if (isValid == true)` is considered poor style in Java, and rewrite it correctly.
3. Convert this nested-`if` logic into an equivalent `else-if` chain, and explain why the chain form better communicates the logic's actual intent: determining a shipping cost tier based on weight ranges.

---

**Previous:** [00 — Module Overview](00-Module-Overview.md) · **Next:** [02 — Switch Statement & Expression](02-Switch-Statement-And-Expression.md)
