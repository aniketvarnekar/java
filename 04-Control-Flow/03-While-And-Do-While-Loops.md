...
1. Write a `do-while` loop that repeatedly generates a random number until it produces a value greater than 90, printing each attempt. Explain why `do-while` is a more natural fit here than `while`.
2. Identify and fix the infinite loop bug in this code: `int n = 1; while (n != 10) { System.out.println(n); n += 2; }` — trace through why it never terminates.
3. Rewrite a `do-while` menu loop (from this topic's example) as an equivalent `while` loop, and explain what extra code duplication was required to preserve the "always show the menu at least once" behavior.

---

**Previous:** [02 — Switch Statement & Expression](02-Switch-Statement-And-Expression.md) · **Next:** [04 — For Loops](04-For-Loops.md)
