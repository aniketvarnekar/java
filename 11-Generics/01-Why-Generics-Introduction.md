...
1. Write a generic `Pair<K, V>` class (as shown in this topic) and demonstrate using it with two different type argument combinations.
2. Rewrite this pre-generics-style code using proper generics, eliminating the manual cast: `List names = new ArrayList(); names.add("Aniket"); String first = (String) names.get(0);`
3. Explain, precisely, why using a raw type (`List` instead of `List<String>`) reintroduces the exact runtime risk generics were designed to eliminate.

---

**Previous:** [00 — Module Overview](00-Module-Overview.md) · **Next:** [02 — Generic Methods](02-Generic-Methods.md)
