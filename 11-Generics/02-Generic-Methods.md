...
1. Write a generic method `<T> boolean isEmpty(T[] array)` returning whether the given array has zero elements, and call it with both a `String[]` and an `Integer[]`.
2. Explain, in your own words, why `findMax`'s type parameter needs the bound `T extends Comparable<T>`, and what compile error would occur without it.
3. Write a generic method `<T> void printAll(List<T> list)` that prints every element, and explain why no bound is needed here (unlike `findMax`).

---

**Previous:** [01 — Why Generics — Introduction](01-Why-Generics-Introduction.md) · **Next:** [03 — Bounded Types & Wildcards](03-Bounded-Types-And-Wildcards.md)
