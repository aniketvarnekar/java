...
1. What's the mechanical difference between checked and unchecked exceptions?
2. Does `finally` run if `try` contains a `return`? What if `finally` also contains a `return`?
3. In what order do multiple try-with-resources resources get closed?
4. What are suppressed exceptions, and when do they occur?
5. Why is an empty `catch` block dangerous?

*(Answers are derivable from Topics 2, 3, 4, 4, and 5 respectively.)*

---

**Previous:** [04 — Try-With-Resources & `AutoCloseable`](04-try-with-resources-and-autocloseable.md) · **Next:** [06 — Module Summary, Interview Questions & Exercises](06-module-summary-exercises.md)
