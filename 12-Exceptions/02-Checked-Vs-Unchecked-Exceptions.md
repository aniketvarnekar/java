...
1. Classify each as checked or unchecked, and explain why: `NullPointerException`, `IOException`, `ArithmeticException`, `SQLException`.
2. Write a method that reads a file (using `FileReader`, which throws checked `IOException`) two ways: once declaring `throws IOException`, once catching it internally — explain the trade-off between these two approaches for the caller.
3. Explain, using a concrete example, why checked exceptions compose poorly with a functional interface like `Comparator<T>`.
4. A colleague proposes making every custom exception in a new project checked, "to be safe." Give a reasoned, balanced response referencing this topic's controversy discussion.

---

**Previous:** [01 — Exception Fundamentals & Hierarchy](01-Exception-Fundamentals-And-Hierarchy.md) · **Next:** [03 — Try-Catch-Finally Deep Dive](03-Try-Catch-Finally-Deep-Dive.md)
