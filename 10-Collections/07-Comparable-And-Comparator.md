...
1. Implement `Comparable<Product>` for a `Product` class (natural order by `price`), then sort a `List<Product>` using `Collections.sort`.
2. Write two different `Comparator<Product>`s (by `name`, and by `price` descending) using modern `Comparator.comparing`/`.reversed()` syntax, and use each to sort the same list differently.
3. Chain a `Comparator` that sorts `Employee`s by `department`, then by `salary` descending as a tiebreaker, using `thenComparing`.
4. Construct a small example where `compareTo` and `equals()` are deliberately inconsistent, and explain the specific, concrete difference this would cause between a `TreeSet<Employee>` and a `HashSet<Employee>` containing the same objects.

---

**Previous:** [06 — Iterators & Iteration](06-Iterators-And-Iteration.md) · **Next:** [08 — The `Collections` Utility Class & Concurrent Collections Preview](08-Collections-Utility-Class-And-Concurrent-Collections-Preview.md)
