...
1. Determine which of these are functional interfaces, and explain why or why not: `interface A { void x(); }`, `interface B { void x(); void y(); }`, `interface C { void x(); default void y() {} }`.
2. Write a custom functional interface `Transformer<T>` with one abstract method `T transform(T input)`, annotated with `@FunctionalInterface`.
3. Explain, in your own words, why a lambda expression's type is always inferred from context (the expected functional interface) rather than being some standalone, universal "function type."

---

**Previous:** [00 — Module Overview](00-module-overview.md) · **Next:** [02 — Lambda Expressions](02-lambda-expressions.md)
