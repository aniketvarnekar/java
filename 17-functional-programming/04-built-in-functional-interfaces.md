...
1. Write a `Function<String, Integer>` and a `Function<Integer, Integer>`, then compose them using both `andThen` and `compose`, predicting each combination's result for the same input.
2. Rewrite a `Predicate<Integer>` checking "positive and even" as two separate named predicates combined with `.and(...)`.
3. Explain why using `IntPredicate` instead of `Predicate<Integer>` avoids autoboxing, referencing Module 03, Topic 6 and Module 11's generics/erasure discussion.

---

**Previous:** [03 — Method References](03-method-references.md) · **Next:** [05 — `Optional`](05-optional.md)
