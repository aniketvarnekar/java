...
1. Implement both singleton variants (double-checked locking and the holder idiom) from memory, and explain out loud why the holder idiom needs no `volatile`.
2. Implement an LRU cache by hand, without `LinkedHashMap` — using a `HashMap` plus a manually maintained doubly-linked list.
3. Rewrite Problem 4's producer-consumer using `java.util.concurrent.BlockingQueue` instead of manual `wait()`/`notify()`, and compare the code length and risk profile.
4. Extend the `Event` class from Problem 6 with a `List<String> tags` field, and correctly defensive-copy it in both the constructor and any accessor.

---

**Previous:** [01 — Core Conceptual Questions — Ranked & Synthesized](01-core-conceptual-questions-ranked.md) · **Next:** [03 — System-Design-Adjacent Java Questions](03-system-design-adjacent-java-questions.md)
