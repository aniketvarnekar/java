...
1. Rewrite a classic `instanceof`-then-cast chain (handling `Integer`, `String`, and `Double`) using `switch` pattern matching with type patterns.
2. Using the `Shape`/`Circle`/`Square`/`Triangle` sealed hierarchy from Topic 2, write an exhaustive `switch` expression computing each shape's area with no `default` branch, and explain what happens if you add a fourth permitted shape without updating this `switch`.
3. Write a nested record pattern that destructures a `Line(Point start, Point end)` directly in a single `case`, extracting all four coordinate values.
4. Explain, from memory, why adding a `default` branch to an otherwise-exhaustive sealed `switch` is a genuine mistake, not just unnecessary code.

---

**Previous:** [02 — Sealed Classes](02-sealed-classes.md) · **Next:** [04 — Modern Java Recap & What's New Through Java 25](04-modern-java-recap-and-whats-new.md)
