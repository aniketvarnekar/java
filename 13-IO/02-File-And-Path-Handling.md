...
1. Write code using `Files` to check if a file exists, and if so, print its size; if not, catch and print a specific, meaningful exception message.
2. Use `Files.walk(...)` to recursively count every `.txt` file within a given directory tree.
3. Explain, with a concrete example, why `Path.resolve("data.csv")` is preferable to manually writing `basePath + "/" + "data.csv"`.

---

**Previous:** [01 — IO Fundamentals, Streams & Encoding](01-IO-Fundamentals-Streams-And-Encoding.md) · **Next:** [03 — Byte Streams & Buffering](03-Byte-Streams-And-Buffering.md)
