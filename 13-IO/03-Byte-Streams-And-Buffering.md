...
1. Write code copying a file byte-by-byte using unbuffered `FileInputStream`/`FileOutputStream`, then rewrite it using `BufferedInputStream`/`BufferedOutputStream` — note that only the construction lines change, not the copy loop itself.
2. Explain, in your own words, precisely why wrapping a stream in a buffer reduces the number of system calls, using this topic's mailbox analogy or one of your own.
3. Explain what design pattern `new BufferedInputStream(new FileInputStream("x"))` demonstrates, and why that pattern allows adding buffering without changing the calling code's interface.

---

**Previous:** [02 — File & Path Handling](02-File-And-Path-Handling.md) · **Next:** [04 — Character Streams, Readers & Writers](04-Character-Streams-Readers-And-Writers.md)
